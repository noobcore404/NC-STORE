...
async function fetchCategoryEmoji() {
  try {
    const url = "https://raw.githubusercontent.com/noobcore404/NC-STORE/912de261e1280f8ac8d5a246e3abb7da4250a25f/nchelp/category.json";
...
function getCategoryFromPath(commandPath) {
  ...
  try {
    ...
    // Get the last directory name (immediate parent folder)
    const category = path.basename(dirName).toLowerCase();

    // If the path is directly in cmds folder, return "general"
    if (dirName.endsWith('cmds') || dirName === '.' || dirName === '') {
      return "general";
    }

    // Return the folder name as category
